...
func (stx *stateTX) State(hash hash.Hash160, s interface{}) error {
	...
	mstate, err := stx.flusher.KVStoreWithBuffer().Get(AccountKVNamespace, hash[:])
...
func (stx *stateTX) PutState(pkHash hash.Hash160, s interface{}) error {
	...
	stx.flusher.KVStoreWithBuffer().MustPut(AccountKVNamespace, pkHash[:], ss)
...
func (stx *stateTX) DelState(pkHash hash.Hash160) error {
	stx.flusher.KVStoreWithBuffer().MustDelete(AccountKVNamespace, pkHash[:])
