...
func (f *flusher) MustPut(ns string, key []byte, value []byte) {
...
func (kvb *kvStoreWithBuffer) Put(ns string, key []byte, value []byte) error {
	kvb.buffer.Put(ns, key, value, "faild to put %x in %s", key, ns)
	return nil
}

func (kvb *kvStoreWithBuffer) MustPut(ns string, key []byte, value []byte) {
	kvb.buffer.Put(ns, key, value, "faild to put %x in %s", key, ns)
}
